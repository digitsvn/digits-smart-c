...
def _get_library_path() -> str:
    ...
    if not lib_path.exists():
        # Fallback for frozen macOS app (PyInstaller usage)
        if getattr(sys, 'frozen', False) and system == 'darwin':
            # Try to locate in Contents/Resources/libs/...
            try:
                # sys.executable -> Contents/MacOS/xiaozhi
                base_dir = Path(sys.executable).parent.parent / "Resources"
                candidate = base_dir / "libs" / "webrtc_apm" / "macos" / arch / "libwebrtc_apm.dylib"
                if candidate.exists():
                    return str(candidate)
            except Exception:
                pass
        
        raise FileNotFoundError(f"Library not found: {lib_path} (Frozen: {getattr(sys, 'frozen', False)})")
