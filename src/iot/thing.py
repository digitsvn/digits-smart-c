...
class ValueType:
    ...
    STRING = "string"
    FLOAT = "float"
    ARRAY = "array"  # Mới thêm
    OBJECT = "object"  # Mới thêm
    LIST = "array"  # LIST là bí danh của ARRAY
...
class Property:
    def __init__(self, name: str, description: str, getter: Callable):
        ...
        self.type = ValueType.STRING  # Kiểu mặc định
        self._type_determined = False

    def _determine_type(self, value: Any):
        """
        Xác định kiểu thuộc tính dựa trên giá trị.
        """
        if isinstance(value, bool):
            ...
            raise TypeError(f"Không hỗ trợ kiểu thuộc tính: {type(value)}")
    ...
    async def get_state_value(self):
        ...
        # Nếu là lần gọi getter đầu tiên, xác định kiểu
...
class Method:
    def __init__(
        ...
        # Bắt buộc callback phải là async function
    ...
    async def invoke(self, params: Dict[str, Any]) -> Any:
        ...
        Gọi phương thức.
        """
        # Thiết lập giá trị tham số, xử lý các kiểu phức tạp
        for name, value in params.items():
            if name in self.parameters:
                param = self.parameters[name]
                # Nếu tham số kiểu STRING nhưng giá trị là dict hoặc list, chuyển thành chuỗi JSON (giống phiên bản C++)
...
class Thing:
    ...
    async def invoke(self, command: Dict) -> Any:
        ...
        if method_name not in self.methods:
            raise ValueError(f"Phương thức không tồn tại: {method_name}")
