...
Phụ trách các chức năng lõi như lưu trữ, truy vấn và cập nhật dữ liệu lịch.
"""
...
class CalendarManager:
    ...
    def __init__(self):
        ...
        # Thử migrate dữ liệu từ file JSON cũ (nếu có)
        self._migrate_from_json_if_exists()

    def init_tools(self, add_tool, PropertyList, Property, PropertyType):
        """
        Khởi tạo và đăng ký toàn bộ tool quản lý lịch.
        """
        from .tools import (
            create_event,
            delete_event,
        ...
        create_event_props = PropertyList(
            [
                ...
                Property("category", PropertyType.STRING, default_value="Mặc định"),
        ...
        add_tool(
            (
                ...
                "- Category 'Nhắc nhở', 'Nghỉ ngơi', 'Đứng dậy': 5 phút\n"
                "- Category 'Họp', 'Công việc': 1 giờ\n"
                "- Tiêu đề chứa 'Nhắc nhở'/'Đứng dậy'/'Nghỉ ngơi': 5 phút\n"
                "- Default: 30 minutes\n"
                "\nArgs:\n"
                ...
                "  category: Event category (Mặc định/Công việc/Cá nhân/Họp/Nhắc nhở)\n"
        ...
        add_tool(
            (
                ...
                "\nFeatures:\n"
                "- Hiển thị thời gian còn lại tới sự kiện (ví dụ: 'sau 2 giờ 30 phút')\n"
    ...
    def _migrate_from_json_if_exists(self):
        """
        Migrate dữ liệu từ file JSON cũ (nếu tồn tại)
        """
        # Kiểm tra file JSON cũ trong project root
        ...
        try:
            ...
            # Nếu không lấy được project root, kiểm tra user cache dir
            user_cache_dir = get_user_cache_dir(create=False)
            json_file = user_cache_dir / "calendar_data.json"

        if os.path.exists(json_file):
            logger.info("Phát hiện file dữ liệu JSON cũ, bắt đầu migrate sang SQLite...")
            if self.db.migrate_from_json(json_file):
                # Sau khi migrate thành công, backup file gốc
                backup_file = f"{json_file}.backup"
                os.rename(json_file, backup_file)
                logger.info(f"Migrate dữ liệu hoàn tất, file gốc đã được backup: {backup_file}")
            else:
                logger.warning("Migrate dữ liệu thất bại, giữ nguyên file JSON")
    ...
    def get_events(
        ...
        try:
            ...
            logger.error(f"Lấy lịch thất bại: {e}")
            return []
...
def get_calendar_manager() -> CalendarManager:
    """
    Lấy singleton của CalendarManager.
    """
