...
class PluginShortcutManager:
    """
    Trình quản lý phím tắt toàn cục tích hợp trong plugin (thay thế views/components/shortcut_manager.py).
    """
    ...
    def __init__(self, loop: Optional[asyncio.AbstractEventLoop]):
        ...
        # Tham chiếu ứng dụng và hiển thị (được inject bởi plugin)
    ...
    async def start(self) -> bool:
        if not self.enabled:
            logger.info("Phím tắt toàn cục đã bị vô hiệu hóa")
            return False
        try:
            from pynput import keyboard
        except Exception as e:
            logger.error(f"Chưa cài đặt thư viện pynput: {e}")
            return False
        ...
        try:
            ...
            logger.info("Đã khởi động trình lắng nghe phím tắt toàn cục")
            return True
        except Exception as e:
            logger.error(f"Khởi động trình lắng nghe phím tắt toàn cục thất bại: {e}")
            return False
    ...
    async def stop(self):
        ...
        try:
            ...
            pass
        logger.info("Đã dừng trình lắng nghe phím tắt toàn cục")
    ...
    async def reload_from_config(self):
        try:
            ...
            self.enabled = bool(self.shortcuts_config.get("ENABLED", True))
            self._load_shortcuts()
            logger.info("Đã tải lại cấu hình phím tắt")
        except Exception as e:
            logger.error(f"T tải lại cấu hình phím tắt thất bại: {e}")
    ...
    async def _health_check_loop(self):
        while self.running and not self._restart_in_progress:
            ...
            # Chỉ nhịp tim nhẹ ở đây; có thể mở rộng nếu cần logic khởi động lại
...
class ShortcutsPlugin(Plugin):
    ...
    async def start(self) -> None:
        if not self._manager:
            return
        # Inject tham chiếu ứng dụng và hiển thị
