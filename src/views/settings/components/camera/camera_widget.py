...
class CameraWidget(QWidget):
    ...
    def _setup_ui(self):
        ...
        try:
            ...
            found = resource_finder.find_file("camera_widget.ui")
            ui_path = found if found else Path(__file__).parent / "camera_widget.ui"
        ...
        self.ui_controls.update(
            {
                ...
                "video_source_combo": self.findChild(QComboBox, "video_source_combo"),
                "video_file_edit": self.findChild(QLineEdit, "video_file_edit"),
                "browse_video_file_btn": self.findChild(
                    QPushButton, "browse_video_file_btn"
                ),
                "video_loop_check": self.findChild(QCheckBox, "video_loop_check"),
    ...
    def _connect_events(self):
        ...
        # Nút chọn file MP4
        if self.ui_controls.get("browse_video_file_btn"):
            self.ui_controls["browse_video_file_btn"].clicked.connect(
                self._on_browse_video_file
            )
    ...
    def _load_config_values(self):
        ...
        try:
            ...
            # Video trong GUI (hiển thị trong khung QML)
            self._set_combo_value(
                "video_source_combo", camera_config.get("VIDEO_SOURCE", "none")
            )
            self._set_text_value(
                "video_file_edit", camera_config.get("VIDEO_FILE_PATH", "")
            )
            self._set_checkbox_value(
                "video_loop_check", bool(camera_config.get("VIDEO_LOOP", True))
            )

        except Exception as e:
            self.logger.error(f"Tải giá trị cấu hình camera thất bại: {e}", exc_info=True)

    def _set_combo_value(self, control_name: str, value: str):
        control = self.ui_controls.get(control_name)
        if not control or not hasattr(control, "findText"):
            return

        target = str(value) if value is not None else ""
        index = control.findText(target)
        if index >= 0:
            control.setCurrentIndex(index)

    def _get_combo_value(self, control_name: str) -> str:
        control = self.ui_controls.get(control_name)
        if control and hasattr(control, "currentText"):
            return control.currentText().strip()
        return ""

    def _set_checkbox_value(self, control_name: str, value: bool):
        control = self.ui_controls.get(control_name)
        if control and hasattr(control, "setChecked"):
            control.setChecked(bool(value))

    def _get_checkbox_value(self, control_name: str) -> bool:
        control = self.ui_controls.get(control_name)
    ...
    def _on_browse_video_file(self):
        """Chọn file MP4 local để phát trong GUI."""
        try:
            file_path, _ = QFileDialog.getOpenFileName(
                self,
                "Chọn file video",
                self._get_text_value("video_file_edit") or str(Path.home()),
                "Video Files (*.mp4 *.mov *.mkv *.avi);;All Files (*)",
            )
            if not file_path:
                return
            self._set_text_value("video_file_edit", file_path)
            self.settings_changed.emit()
        except Exception as e:
            self.logger.error(f"Chọn file video thất bại: {e}", exc_info=True)
    ...
    def get_config_data(self) -> dict:
        ...
        try:
            ...
            # Video GUI
            camera_config["VIDEO_SOURCE"] = self._get_combo_value("video_source_combo")
            camera_config["VIDEO_FILE_PATH"] = self._get_text_value("video_file_edit")
            camera_config["VIDEO_LOOP"] = self._get_checkbox_value("video_loop_check")
    ...
    def reset_to_defaults(self):
        ...
        try:
            ...
            # Video GUI
            self._set_combo_value("video_source_combo", camera_config["VIDEO_SOURCE"])
            self._set_text_value("video_file_edit", camera_config["VIDEO_FILE_PATH"])
            self._set_checkbox_value("video_loop_check", camera_config["VIDEO_LOOP"])
